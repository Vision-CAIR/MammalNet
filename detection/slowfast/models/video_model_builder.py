...
class SlowFast_czsl(nn.Module):
    ...
    def __init__(self, cfg):
        ...
        Args:
            cfg (CfgNode): model building configs, details are in the
                comments of the config file.
        """
        super(SlowFast_czsl, self).__init__()
        self.norm_module = get_norm(cfg)
        self.cfg = cfg
        self.enable_detection = cfg.DETECTION.ENABLE
        self.num_pathways = 2
        ...
        self.animal_index = animals
        self.behavior_index = behaviors
        ...
        self.behavior_embedder = nn.Embedding(behavior_emb.shape[0], behavior_emb.shape[1])
    ...
    def _construct_network(self, cfg):
        ...
        # added
        self.projection = nn.Linear(300 * 2, 400)
    ...
    # def get_paired_index(self, animal_label, behavior_label):

    #     print("pair 2 idx",self.pair2idx)  
    #     pass

    def compose(self, animals, behaviors):
        # animals, behaviors = self.animal_embedder(animal), self.behavior_embedder(behavior)
        inputs = torch.cat([animals, behaviors], 1)
    ...
    def forward(self, x, bboxes=None):
        ...
        # self.get_paired_index(animal_label,behavior_label)



        # print(type(x))
        # print(type(self.pair2idx), len(self.pair2idx))

        # print(type(self.animal_tensor))
        pair_embed = self.compose(self.animal_tensor, self.behavior_tensor).permute(1,0)
        x = F.normalize(x, dim=1)

        # pair_embed = self.compose(self.train_attrs, self.train_objs).permute(1,0)
        pair_pred = torch.matmul(x, pair_embed)
...
class SlowFast_composition_czsl(nn.Module):
    ...
    def forward(self, x, animal_label=None, behavior_label=None, bboxes=None):
        ...
        # print(animal_prediction.shape, behavior_prediction.shape)


        # print(behavior_prediction)
        
        animal_prediction = F.normalize(animal_prediction,dim=1)
        behavior_prediction = F.normalize(behavior_prediction, dim=1)


        # print(animal_label, behavior_label)


        # if animal_label is not None:
        #     # print(type(animal_label), type(behavior_label), animal_label.dtype, behavior_label.dtype)
        #     animal_tensor = F.normalize(self.animal_embedder(animal_label), dim=1).permute(1,0)
        #     behavior_tensor = F.normalize(self.behavior_embedder(behavior_label), dim=1).permute(1,0)

        # else:
        #     print(type(animal_label), type(behavior_label))


        #     animal_tensor = F.normalize(self.animal_tensor,dim=1).permute(1,0)
        #     behavior_tensor = F.normalize(self.behavior_tensor, dim=1).permute(1,0)


        animal_tensor = F.normalize(self.animal_embedder(self.animal_index), dim=1).permute(1,0)
        behavior_tensor = F.normalize(self.behavior_embedder(self.behavior_index), dim=1).permute(1,0)


        # print(animal_prediction.shape, behavior_prediction.shape, animal_prediction.shape,animal_tensor.shape)
        ...
        # print("behavior prediction similarity", behavior_prediction)
...
class SlowFast_composition(nn.Module):
    ...
    def _construct_network(self, cfg):
        ...
        if cfg.DETECTION.ENABLE:
            ...
            # self.linear_behavior_head = nn.linear(400,300)
            # self.linear_animal_head = nn.linear(400,300)
...
@MODEL_REGISTRY.register()
class MViT_composition(nn.Module):
    """
    Model builder for MViTv1 and MViTv2.
    ...
    def __init__(self, cfg):
        ...
        if self.enable_detection:
            ...
            self.animal_head = head_helper.TransformerBasicHead(
                2 * embed_dim
                if ("concat" in cfg.MVIT.REV.RESPATH_FUSE and self.enable_rev)
                else embed_dim,
            ...
            self.behavior_head = head_helper.TransformerBasicHead(
                2 * embed_dim
                if ("concat" in cfg.MVIT.REV.RESPATH_FUSE and self.enable_rev)
                else embed_dim,
        ...
        self.animal_head.projection.weight.data.mul_(head_init_scale)
        self.animal_head.projection.bias.data.mul_(head_init_scale)


        self.behavior_head.projection.weight.data.mul_(head_init_scale)
        self.behavior_head.projection.bias.data.mul_(head_init_scale)
    ...
    def _forward_reversible(self, x):
        ...
        # x = self.head(x)
        behavior_prediction = self.behavior_head(x)
        animal_prediction = self.animal_head(x)
