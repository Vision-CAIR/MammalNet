...
def sample_frame_index(total_length):
    if total_length <600:
        return np.linspace(0,total_length-1, total_length, dtype=int)
    elif total_length <2000:
        return np.linspace(0,total_length-1, 400, dtype=int)
    else:
        return np.linspace(0,total_length-1, 600, dtype=int)
    ...
    if task == 'rgb':
        if args.use_opencv:
            ...
            try:
                ...
                sampled_index = sample_frame_index(len(vr))
...
def parse_args():
    ...
    parser.add_argument('--num-gpu', type=int, default=4, help='number of GPU')
    parser.add_argument(
        '--resume',
        action='store_true',
        default=False,
