...
def topks_correct_composition(animal_preds, animal_labels, behavior_preds, behavior_labels, ks):
    ...
    # print("animal label, behavior label",animal_labels,behavior_labels)
    # (batch_size, max_k) -> (max_k, batch_size).
    top_max_k_inds = top_max_k_inds.t()
    # (batch_size, ) -> (max_k, batch_size).
    rep_max_k_labels = animal_labels.view(1, -1).expand_as(top_max_k_inds)

    # print("rep max k labels shape",rep_max_k_labels.shape)
    # print("rep max k labels", rep_max_k_labels)
    # print("top max k inds",top_max_k_inds)

    # (i, j) = 1 if top i-th prediction for the j-th sample is correct.
    top_max_k_correct_animal = top_max_k_inds.eq(rep_max_k_labels)
    # print("top_max_k_correct_animal",top_max_k_correct_animal)
    # Compute the number of topk correct predictions for each k.
    ...
    # print("top max k correct animal",top_max_k_correct_animal)
    # print("top max k correct behavior", top_max_k_correct_behavior)

    top_max_k_coposition = top_max_k_correct_behavior.float() + top_max_k_correct_animal.float()

    # print("topk correct composition", top_max_k_coposition)
